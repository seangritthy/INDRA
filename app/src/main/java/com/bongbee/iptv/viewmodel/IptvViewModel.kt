...
class IptvViewModel : ViewModel() {
    ...
    val countries = listOf(
        ...
        Country("Laos", "https://iptv-org.github.io/languages/khm.m3u"),
    ...
    private val tmdbApiKey = "5e10bf06e4f15dae6e9ff35ff35e8df2"
    ...
    // Watch History State
    var watchHistory by mutableStateOf<List<WatchHistoryItem>>(emptyList())
        private set
                            ...
                            for (j in 0 until resultsArray.length()) {
                                ...
                                if (mediaType == "person") {
                                    ...
                                    if (knownFor != null) {
                                        for (k in 0 until knownFor.length()) {
                                            ...
                                            movies.add(Movie(
                                                ...
                                                releaseDate = item.optString("release_date", item.optString("first_air_date", null)),
                                    ...
                                    movies.add(Movie(
                                        ...
                                        releaseDate = obj.optString("release_date", obj.optString("first_air_date", null)),
                                        mediaType = mediaType
                                    ))
                                }
                            }
    ...
    fun addToWatchHistory(movie: Movie) {
        ...
        val existingIndex = watchHistory.indexOfFirst { it.id == movie.id && it.mediaType == movie.mediaType }
        ...
        watchHistory = if (existingIndex >= 0) {
            // Move existing item to the top (most recently watched)
            val mutableList = watchHistory.toMutableList()
            mutableList.removeAt(existingIndex)
            listOf(newItem) + mutableList
        } else {
            // Add new item to the top
            listOf(newItem) + watchHistory
        }
    }

    fun removeFromWatchHistory(historyItem: WatchHistoryItem) {
        watchHistory = watchHistory.filter { it.id != historyItem.id || it.mediaType != historyItem.mediaType }
    }
